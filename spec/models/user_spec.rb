...
RSpec.describe User, type: :model do
  it {
    expect(subject).to respond_to :admin, :email, :encrypted_password,
                                  :first_name, :last_name, :remember_created_at,
                                  :reset_password_sent_at, :reset_password_token
  }

  it {
    expect(subject).to have_db_column(:admin).of_type(:boolean).with_options(null: false)
    ...
    expect(subject).to have_db_column(:first_name).of_type(:string)
    expect(subject).to have_db_column(:last_name).of_type(:string)
